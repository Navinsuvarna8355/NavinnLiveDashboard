...
def fetch_option_chain(symbol_key, current_minute_key):
    """
    Fetches option chain data for a given symbol.
    A unique `current_minute_key` is used to force a cache refresh every minute.
    """
...
if "data_container" not in st.session_state:
    ...
    st.session_state.decay_side = "Loading..."
...
with col1:
    ...
    # Check if a new fetch is needed based on manual button, symbol change, or auto-refresh time
    if fetch_button or selected_symbol != st.session_state.selected_symbol or \
       (auto_refresh and (current_time - st.session_state.last_fetch_time >= refresh_rate)):
        
        st.session_state.selected_symbol = selected_symbol
        with st.spinner(f"Fetching live data for {selected_symbol}..."):
            data_dict = fetch_option_chain(selected_symbol, int(current_time // 60))
            if data_dict:
        ...
        # Rerun to update the entire page with the new data
    ...
    if st.session_state.data_container:
        ...
        # Store decay_side and dataframe in session state for access in other columns
        st.session_state.decay_side = decay_side
        st.session_state.df = df
        
        st.metric("Decay Side", st.session_state.decay_side)
...
with col2:
    ...
    if "df" in st.session_state:
        ...
        with tab2:
            chart_fig = create_decay_chart(st.session_state.df)
...
if "decay_side" in st.session_state and st.session_state.decay_side != "Loading...":
    decay_side = st.session_state.decay_side
